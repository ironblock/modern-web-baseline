...
export type NormalFSA<Payload: mixed | Error = typeof undefined, Meta: mixed = typeof undefined> = {|
  type: ActionType,
  payload: Payload,
  error: boolean,
  meta: Meta
...
export type JSONObject = ({ [key: string]: PrimitiveType | JSONObject | JSONArray | void } & { $call?: void });
export type JSONArray = Array<PrimitiveType> | Array<JSONObject> | Array<JSONArray>;
...
export const handleSuccessKeyed = <Name: string, State: KeyedReducerState<Name>>(
  name: Name,
  state: State,
  action: FSA<*, *>,
  key: string,
  ): State => (
  {
    ...
    [name]: {
      ...
      lastUpdate: Date.now(),
      lastResult: 'success',
      collection: handleKeyedCollection(key, state[name].collection, action.payload),
    },
  }
);
